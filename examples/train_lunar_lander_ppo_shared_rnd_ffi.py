"""Train LunarLander-v3 with parallel PPO agents sharing one FFI RND predictor."""
...
def train(args) -> None:
    ...
    curiosity = create_rnd(
        lib,
        rnd_config,
        rnd_path(args.save_path, "shared"),
        rnd_path(args.load_path, "shared"),
    )
    agents = []
    try:
...
def main() -> None:
    ...
    parser.set_defaults(parallel=4)
