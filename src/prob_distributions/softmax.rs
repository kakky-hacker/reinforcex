...
impl SoftmaxDistribution {
    pub fn new(logits: Tensor, beta: f64, min_prob: f64) -> Self {
        let n: f64 = logits.size()[1] as f64;
    ...
    fn all_prob(&self) -> Tensor {
        let scaled_logits: Tensor = &self.logits * self.beta;
        if self.min_prob > 0.0 {
            let softmax: Tensor = scaled_logits.softmax(-1, tch::Kind::Float);
...
impl BaseDistribution for SoftmaxDistribution {
    ...
    fn kl(&self, q: Box<dyn BaseDistribution>) -> Tensor {
        let q_log_prob: Tensor = q.log_prob(&self.all_prob());
    ...
    fn sample(&self) -> Tensor {
        let probs: Tensor = self.all_prob();
        let noise: Tensor = Tensor::rand(&probs.size(), (tch::Kind::Float, probs.device())).log() * -1.0;
        let logits_with_noise: Tensor = (probs.log() + noise).argmax(-1, false);
        logits_with_noise
    }
...
mod tests {
    ...
    #[test]
    fn test_all_prob() {
        let logits: Tensor = Tensor::from_slice(&[1.0, 2.0, 3.0, 4.0]).reshape(&[1, 4]);
        let dist: SoftmaxDistribution = SoftmaxDistribution::new(logits, 1.0, 0.0);

        // Test probabilities sum to 1
        let all_prob: Tensor = dist.all_prob();
    ...
    fn test_all_prob_with_min_prob() {
        let logits: Tensor = Tensor::from_slice(&[1.0, 2.0, 3.0, 4.0]).reshape(&[1, 4]);
        let beta: f64 = 1.0;
        let min_prob: f64 = 0.1;
        let dist: SoftmaxDistribution = SoftmaxDistribution::new(logits, beta, min_prob);

        // Ensure minimum probability constraint is applied
        let all_prob: Tensor = dist.all_prob();
        let min_val: f64 = all_prob.min().double_value(&[]);
    ...
    fn test_all_log_prob() {
        ...
        let all_log_prob: Tensor = dist.all_log_prob();
        let log_sum: f64 = all_log_prob.sum(Kind::Float).double_value(&[]);
    ...
    fn test_prob() {
        let logits: Tensor = Tensor::from_slice(&[1.0, 2.0, 3.0, 4.0]).reshape(&[1, 4]);
        let dist: SoftmaxDistribution = SoftmaxDistribution::new(logits, 1.0, 0.0);
        let prob: Tensor = dist.prob(&Tensor::from_slice(&[0 as i64]).reshape(&[1, 1]));
        assert!((prob.double_value(&[]) - 0.032058604).abs() < 1e-6);
        let prob: Tensor = dist.prob(&Tensor::from_slice(&[1 as i64]).reshape(&[1, 1]));
        assert!((prob.double_value(&[]) - 0.087144318).abs() < 1e-6);
        let prob: Tensor = dist.prob(&Tensor::from_slice(&[2 as i64]).reshape(&[1, 1]));
        assert!((prob.double_value(&[]) - 0.236882818).abs() < 1e-6);
        let prob: Tensor = dist.prob(&Tensor::from_slice(&[3 as i64]).reshape(&[1, 1]));
    ...
    fn test_log_prob() {
        let logits: Tensor = Tensor::from_slice(&[1.0, 2.0, 3.0, 4.0]).reshape(&[1, 4]);
        let dist: SoftmaxDistribution = SoftmaxDistribution::new(logits, 1.0, 0.0);
        let log_prob: Tensor = dist.log_prob(&Tensor::from_slice(&[0 as i64]).reshape(&[1, 1]));
        assert!((log_prob.double_value(&[]) - (-3.440189702)).abs() < 1e-6);
        let log_prob: Tensor = dist.log_prob(&Tensor::from_slice(&[1 as i64]).reshape(&[1, 1]));
        assert!((log_prob.double_value(&[]) - (-2.440189702)).abs() < 1e-6);
        let log_prob: Tensor = dist.log_prob(&Tensor::from_slice(&[2 as i64]).reshape(&[1, 1]));
        assert!((log_prob.double_value(&[]) - (-1.440189702)).abs() < 1e-6);
        let log_prob: Tensor = dist.log_prob(&Tensor::from_slice(&[3 as i64]).reshape(&[1, 1]));
    ...
    fn test_entropy() {
        let logits: Tensor = Tensor::from_slice(&[1.0, 2.0, 3.0, 4.0]).reshape(&[1, 4]);
        let dist: SoftmaxDistribution = SoftmaxDistribution::new(logits, 1.0, 0.0);
        let entropy: Tensor = dist.entropy();
    ...
    fn test_most_probable() {
        let logits: Tensor = Tensor::from_slice(&[1.0, 2.0, 3.0, 4.0]).reshape(&[1, 4]);
        let dist: SoftmaxDistribution = SoftmaxDistribution::new(logits, 1.0, 0.0);
        let most_probable: Tensor = dist.most_probable();
        assert_eq!(most_probable.int64_value(&[]), 3);

        let logits: Tensor = Tensor::from_slice(&[1.0, 3.5, 1.0, 2.0]).reshape(&[1, 4]);
        let dist: SoftmaxDistribution = SoftmaxDistribution::new(logits, 1.0, 0.1);
        let most_probable: Tensor = dist.most_probable();
        assert_eq!(most_probable.int64_value(&[]), 1);

        let logits: Tensor = Tensor::from_slice(&[1.0, 3.5, 5.0, 2.0]).reshape(&[1, 4]);
        let dist: SoftmaxDistribution = SoftmaxDistribution::new(logits, 2.0, 0.1);
        let most_probable: Tensor = dist.most_probable();
        assert_eq!(most_probable.int64_value(&[]), 2);

        let logits: Tensor = Tensor::from_slice(&[5.1, 3.5, 5.0, 4.0]).reshape(&[1, 4]);
        let dist: SoftmaxDistribution = SoftmaxDistribution::new(logits, 1.5, 0.0);
        let most_probable: Tensor = dist.most_probable();
